...
public class Personaje : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        idle.OnEnter += x => { Debug.Log("Rama estas tarde"); };
        ...
        //MOVING
        moving.OnEnter += x => { _myRen.material.color = Color.blue; };
        moving.OnUpdate += () =>
        {
            if (Input.GetAxis("Horizontal") == 0 && Input.GetAxis("Vertical") == 0)
        ...
        //DIE
        die.OnEnter += x => { _myRen.material.color = Color.gray; };
        ...
        jumping.GetTransition(PlayerInputs.JUMP).OnTransition += x => { _myRen.material.color = Color.green; };
        //En cambio si estamos en "jumping" y se le pone el input de PlayerInputs.IDLE se ejecutaria esto
        jumping.GetTransition(PlayerInputs.IDLE).OnTransition += x => { _myRen.material.color = Color.white; };

        //En cambio si estamos en "jumping" y se le pone el input de PlayerInputs.IDLE se ejecutaria esto
        moving.GetTransition(PlayerInputs.IDLE).OnTransition += x => { _myRen.material.color = Color.black; };

        high.GetTransition(PlayerInputs.DIE).OnTransition += x => { Debug.LogWarning("F"); };
